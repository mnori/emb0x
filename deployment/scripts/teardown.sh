...
if [ -f data/instance-id.txt ]; then
    INSTANCE_ID=$(cat data/instance-id.txt)
    aws ec2 terminate-instances --instance-ids "$INSTANCE_ID" --region "$AWS_REGION"
else
    echo "data/instance-id.txt not found. Skipping EC2 teardown."
fi

# Security group teardown
if [ -f data/security-group-id.txt ]; then
    ...
    echo "data/security-group-id.txt not found. Skipping security group teardown."
fi

# Subnet teardown - this must happen prior to VPC teardown
if [ -f data/subnet-id.txt ]; then
    SUBNET_ID=$(cat data/subnet-id.txt)
    aws ec2 delete-subnet --subnet-id "$SUBNET_ID" --region "$AWS_REGION"
else
    echo "data/subnet-id.txt not found. Skipping subnet teardown."
fi
...
if [ -f vpc-id.txt ]; then
    ...
    echo "data/vpc-id.txt not found. Skipping VPC teardown."
fi
